...
# It uses the OpenAI client to interact with the GPT-4 model.
...
def query_gpt(prompt):
    response = client.chat.complete(
        ...
        messages=[
            {"role": "system", "content": """You are an expert Python code reviewer, AI debugger, and the senior-most Quality Assurance Engineer.Please:
    ...
    Use negative assertions to ensure the code is safe and secure.            

    Strictly follow the Json format below for your responses:
...
Example Response
{ LineNo: 42,
RiskExplaination: “Using `eval()` on untrusted user input is highly dangerous. It allows execution of arbitrary Python code, which may lead to security breaches, data loss, or remote code execution. Even seemingly benign input can exploit system access or leak sensitive data.”,
EdgeCases: “
...
AlternativeApproach: “Safe alternative using `literal_eval` to avoid executing arbitrary code”,
...
{ LineNo: 18,
RiskExplaination: “Directly accessing a query parameter using 'request.GET['id']' can raise a 'KeyError' if the 'id' parameter is missing from the request. This can crash the application or expose an unhandled exception to users. It also 
...
”,
AlternativeApproach: “Input is a non-numeric or unexpected string → validate format if id is expected to be an integer.”,
CorrectedCode: “
data = request.GET.get('id', None)
data = request.GET['id']
”
...
def debug_issues(risks: dict):
    result = []
    for lineno, snippet in risks.items():
        prompt = f"""
